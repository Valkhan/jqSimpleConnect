 ...
 * @returns {boolean} 'true' if the operation was done, 'false' if the connection no exists.
 */
jqSimpleConnect.hideConnection = function (connectionId) {
    if (jqSimpleConnect._connections[connectionId] != null) {
        // Remove HTML element.
        jQuery('.jqSimpleConnect.' + connectionId).hide();
        return true;
    }
    return false;
}
...
jqSimpleConnect.hideConnections = function (connectionId) {
    for (var connId in connectionId) {
        jqSimpleConnect.hideConnection(connectionId[connId]);
    }
}
...
jqSimpleConnect.showConnections = function (connectionId) {
    for (var connId in connectionId) {
        jqSimpleConnect.showConnection(connectionId[connId]);
    }
}
...
/**
 * Added listener to window resize event.
 */
